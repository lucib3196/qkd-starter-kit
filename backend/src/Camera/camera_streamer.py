...
# Should return the camera folder
...
class CalibratedCamera:
    ...
    def calibrate_camera(self):
        # Use self.frame_height instead of self.height
        self.new_cam_mtx, self.roi = cv2.getOptimalNewCameraMatrix(
            self.camera_matrix, self.camera_dist, (self.frame_width, self.frame_height), 1, (self.frame_width, self.frame_height)
        )
...
class PiVideo:
    def __init__(self, calibrated_camera: CalibratedCamera, framerate=60, format="XBGR8888",brightness=0.0, contrast=0.8, saturation=1.3):
        ...
        self.frame_count = 0
         ...
         # Clients ie who is viewing the stream
        self.clients = 0
        ...
        config = self.picam2.create_video_configuration(
            main={"size": (self.width,self.height), "format": format},
            controls={
        ...
        # Intialize the frame buffer
        self.buffer = io.BytesIO()
        self.frame_buffer = None
        
        # config["network_file"] = "/usr/share/imx500-models/imx500_mobilenet_ssd.json"
    ...
    def start(self):
        self.picam2.start()
        self.capture_thread = threading.Thread(target = self._capture_frames, daemon=True, name='CaptureThread'
                                                )
    ...
    def _capture_frames(self):
        ...
        while not self.stop_event.is_set():
            try:
                ...
                # Only capture if we have clients or no frames
                if self.clients > 0 or self.frame_buffer is None:
                    ...
                    buffer = buffers[0]
                    image_array = self.picam2.helpers.make_array(buffer, self.picam2.camera_configuration()["main"])
                    ret, jpeg = cv2.imencode('.jpg', self.calibrated_camera.undistort_frame(image_array))
        ...
        try:
            while True:
                ...
                if frame_data is not None:
                    # print(f"This is the frame data {frame_data}\n\n")
                    yield (b'--frame\r\n'
